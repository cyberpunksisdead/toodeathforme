...
def test_logger_info_messages(caplog):
    ...
    assert "Admin panel (en) mounted at /admin/en" in caplog.text
    assert "Admin panel (ru) mounted at /admin/ru" in caplog.text
    assert "/admin redirects to /admin/en" in caplog.text
    assert "Available locales: en, ru" in caplog.text
    assert "Access at: http://localhost:8000/admin/en" in caplog.text
