...
class MarkdownFileManager:
  ...
  def load_post(self, slug: str) -> Optional[MarkdownPost]:
    ...
    posts_data = [
      {
        ...
        'tags': ', '.join(p.tags) if isinstance(p.tags, list) else str(p.tags) if p.tags else '',
      }
      for p in posts
    ]
  ...
  async def render(self, request: Request, templates: Jinja2Templates) -> Response:
    ...
    if not post:
      return templates.TemplateResponse(
        request=request,
        name='admin/404.html',
        context={'title': 'Post not found', 'message': f'Post "{slug}" not found'},
        status_code=404,
      )
