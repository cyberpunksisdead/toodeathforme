...
Отлично! Теперь изучил `/app/docs/TODO.md`. Это подробное техническое задание по устранению технического долга. Дам оценку каждого пункта:
...
#### ✅ **1.1 Firebase secrets**
- **Резонность**: 10/10 - утечка секретов = критическая уязвимость
- **Статус**: ⚠️ Требует проверки коммита `80e1fa0`
- **Рекомендация**: **ПРОВЕРИТЬ СЕЙЧАС**. Если есть реальные ключи - немедленно отозвать и удалить из истории

#### ⚠️ **1.2 Исправить тест test_role_view_accessible_only_for_root_user**
- **Резонность**: 8/10 - ложноположительный тест = скрытые баги
- **Статус**: ❌ Mock проверяет `app.state`, а код читает `self.admin_username`
- **Рекомендация**: **ИСПРАВИТЬ**. Тест не проверяет реальное поведение

---

### 🟡 **ВАЖНО - СДЕЛАТЬ В БЛИЖАЙШЕЕ ВРЕМЯ**

#### **2.1 Зачистить deprecated параметры**
- **Резонность**: 7/10 - устраняет confusion, упрощает API
- **Статус**: ⚠️ 4 deprecated параметра висят с warnings
- **Рекомендация**: **СДЕЛАТЬ**. Наметить версию удаления (v0.9.0)

#### **2.2 Унифицировать точки входа**
- **Резонность**: 6/10 - улучшает DX, но не критично
- **Статус**: ❌ Две независимые функции без валидации порядка
- **Рекомендация**: **МОЖНО ОТЛОЖИТЬ**. Текущий API работает, breaking change нежелателен

#### **2.3 i18n для blog-роутов**
- **Резонность**: 5/10 - Nice to have, но не блокер
- **Статус**: ❌ Blog только на английском
- **Рекомендация**: **НИЗКИЙ ПРИОРИТЕТ**. Сначала закрыть критичные задачи

#### **2.4 Унифицировать аутентификацию**
- **Резонность**: 7/10 - упрощает архитектуру, улучшает security
- **Статус**: ❌ Cookie-сессия для admin, отдельный механизм для API
- **Рекомендация**: **СДЕЛАТЬ**. Единый `get_current_user` dependency

---

### 🟢 **КАЧЕСТВО КОДА - МОЖНО ДЕЛАТЬ ПОСТЕПЕННО**

#### **3.1 Убрать print-логи**
- **Резонность**: 9/10 - пароль в stdout = security issue
- **Статус**: ❌ `print("✓ Login: username='admin' password='Admin123!')` в коде
- **Рекомендация**: **СДЕЛАТЬ СРОЧНО**. Это security leak

#### **3.2 Тесты на theme switcher**
- **Резонность**: 6/10 - защита от регрессий
- **Статус**: ❌ Только примеры, нет unit-тестов
- **Рекомендация**: **СДЕЛАТЬ**. Быстрая задача, 6 простых тестов

#### **3.3 Ужесточить безопасность defaults**
- **Резонность**: 8/10 - weak secret_key = уязвимость
- **Статус**: ❌ Нет валидации энтропии secret_key
- **Рекомендация**: **СДЕЛАТЬ**. Добавить warning на слабые ключи

---

### 📝 **ДОКУМЕНТАЦИЯ - ВАЖНО ДЛЯ ONBOARDING**

#### **4.1 Синхронизировать README**
- **Резонность**: 7/10 - мёртвые ссылки = плохое впечатление
- **Статус**: ❌ Ссылки на старый репо `awestley/`
- **Рекомендация**: **СДЕЛАТЬ**. Быстро, улучшает первое впечатление

#### **4.2 Актуализировать QUICKSTART**
- **Резонность**: 8/10 - новый разработчик не может запустить проект
- **Статус**: ⚠️ Нет альтернатив без uv, нет env-переменных
- **Рекомендация**: **СДЕЛАТЬ**. Критично для onboarding

#### **4.3 CONTRIBUTING.md и структурированный changelog**
- **Резонность**: 6/10 - улучшает collaboration
- **Статус**: ❌ Нет CONTRIBUTING.md
- **Рекомендация**: **МОЖНО ОТЛОЖИТЬ**. Не блокер для разработки

---

## 🎯 ИТОГОВЫЕ РЕКОМЕНДАЦИИ

### **Делать СЕЙЧАС** (критично):
1. ✅ **1.1** - Проверить Firebase secrets
2. ✅ **3.1** - Убрать print с паролем (security leak!)
3. ✅ **1.2** - Исправить ложноположительный тест

### **Делать В ТЕЧЕНИЕ НЕДЕЛИ** (важно):
4. ✅ **2.1** - Зачистить deprecated параметры
5. ✅ **2.4** - Унифицировать аутентификацию
6. ✅ **3.3** - Валидация weak secret_key
7. ✅ **4.1** - Обновить README с правильными ссылками
8. ✅ **4.2** - Улучшить QUICKSTART
9. ✅ **3.2** - Добавить тесты на theme switcher

### **Можно отложить** (не критично):
10. 🟡 **2.2** - Unified entry point (breaking change)
11. 🟡 **2.3** - i18n для blog
12. 🟡 **4.3** - CONTRIBUTING.md (nice to have)

---

## ❌ ЧТО НЕ НУЖНО ДЕЛАТЬ

- **НЕ унифицировать точки входа** (2.2) сейчас - это breaking change, требует major version bump
- **НЕ делать blog i18n** (2.3) до закрытия критичных задач - это feature, не bugfix

---

**Общая оценка TODO.md**: 9/10 - очень качественный документ! Чётко приоритизировано, с метриками и порядком выполнения. Единственное замечание - задачу 2.2 (unified entry point) лучше отложить до v1.0, так как это breaking change.

## 📋 Резюме анализа TODO.md

Документ очень качественный и хорошо структурированный. Вот краткая сводка по приоритетам:

### 🔴 **КРИТИЧНО** (сделать немедленно):
1. **Проверить Firebase secrets** (1.1) - потенциальная утечка API-ключей
2. **Убрать print с паролем** (3.1) - пароль выводится в stdout
3. **Исправить ложный тест** (1.2) - тест проверяет не то, что реально работает

### 🟡 **ВАЖНО** (в течение недели):
4. Зачистить deprecated параметры (2.1)
5. Унифицировать аутентификацию (2.4)
6. Валидация weak secret_key (3.3)
7. Обновить README (4.1)
8. Улучшить QUICKSTART (4.2)
9. Тесты на theme switcher (3.2)
...
НЕ ВЫПОЛНЯТЬ, НО ПРОВЕРИТЬ НЕОБХОДИМОСТЬ:
